exports.yargs = {
    ...
    handler: async(argv) => {
        ...
        buster.on('response', (res) => {
            ...
            const contentLength = colors.bold.white(res.responseHeaders['content-length'] || '-')
            const time = (res.info.stopTime - res.info.startTime).toFixed(2)
            const location = res.responseHeaders['location'] ? `-> ${res.responseHeaders['location']}` : ''

            console.warn(`${res.uri} -> ${responseCode} ${contentType} ${contentLength} ${time}ms ${location}`)
        ...
        buster.on('test-response', (res) => {
            ...
            console.warn(`Test ${res.uri} -> ${responseCode} ${contentType} ${contentLength} ${time}ms ${location}`)
        })
