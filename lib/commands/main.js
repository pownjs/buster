exports.yargs = {
    ...
    handler: async(argv) => {
        ...
        buster.on('response', (res) => {
            console.info(`${res.uri} -> ${responseCodeColorFuncs[~~(res.responseCode/100) % 10](res.responseCode)} ${colors.cyan(res.responseHeaders['content-type'] || '-')} ${(res.info.stopTime - res.info.startTime).toFixed(2)}ms`)

            results.push(res)
        })

        buster.on('test-response', (res) => {
            console.warn(`Test ${res.uri} -> ${responseCodeColorFuncs[~~(res.responseCode/100) % 10](res.responseCode)} ${colors.cyan(res.responseHeaders['content-type'] || '-')} ${(res.info.stopTime - res.info.startTime).toFixed(2)}ms`)
        })
