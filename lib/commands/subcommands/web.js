exports.yargs = {
    ...
    builder: (yargs) => {
        ...
        yargs.example(`$0 buster -b --all -n words.txt http://target`, 'Start buster but also open the results in nice text user interface')
    },

    handler: async(argv) => {
        const { write, requestMethod, nameDictionary, extensionDictionary, namePrefix, nameSuffix, extensionPrefix, extensionSuffix, requestConcurrency, loadConcurrency, header, timeout, all, yes, no, blessed, url } = argv
